...
import 'package:jetleaf_lang/reflection.dart' show RuntimeScannerConfiguration;
...
import 'jet_runtime_stub_scanner.dart'
    if (dart.library.mirrors) 'package:jetleaf_lang/reflection.dart';
...
///
/// Logging integration:
/// - **Info** messages are logged with [Log.info], or printed to stderr
///   if info-level logging is disabled.
/// - **Warnings** are logged with [Log.warn], or printed to stderr
///   if warn-level logging is disabled.
/// - **Errors** are logged with [Log.error], or printed to stderr
...
/// {@endtemplate}
Future<RuntimeProvider> runScan(Log logger) async {
  final scan = await ApplicationRuntimeScanner(
    onInfo: (msg) {
